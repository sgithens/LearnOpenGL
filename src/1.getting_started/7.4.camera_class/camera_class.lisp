...
(defun camera-class (&key (vertex-shader-file "src/1.getting_started/7.4.camera_class/7.4.camera.vs")
                          (fragment-shader-file "src/1.getting_started/7.4.camera_class/7.4.camera.fs")
    ...
    (let* ((our-shader (make-shader vertex-shader-file
      ...
      (loop until (cl-glfw3:window-should-close-p)
          do (progn
              ...
              ;; pass projection matrix to shader (note that in this case it could change every fraem)
              (set-mat4 our-shader "projection"
              (3d-matrices:mperspective (zoom camera) (/ *scr-width* *scr-height*) 0.1 100.0))

              ;; camera/view transformation
              (set-mat4 our-shader "view" (get-view-matrix camera))
