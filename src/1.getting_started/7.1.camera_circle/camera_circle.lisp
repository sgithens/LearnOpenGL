...
(defun camera-circle (&key (vertex-shader-file "src/1.getting_started/7.1.camera_circle/7.1.camera.vs")
                           (fragment-shader-file "src/1.getting_started/7.1.camera_circle/7.1.camera.fs")
      ...
      (loop until (cl-glfw3:window-should-close-p)
          do (progn
              ...
              (let* ((time   (%cl-glfw3:get-time))
                     ...
                     (radius 10.0)
                     (cam-x  (* (sin time) radius))
                     (cam-z  (* (cos time) radius)))

                ;; camera/view transformation
                (setq view (3d-matrices:nmlookat view
                                                 (3d-vectors:vec cam-x 0.0 cam-z)
                                                 (3d-vectors:vec 0.0 0.0 0.0)
                                                 (3d-vectors:vec 0.0 1.0 0.0)))
