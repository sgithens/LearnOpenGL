...
(defun transformations-exercise1 (&key (vertex-shader-file "src/1.getting_started/5.1.transformations/5.1.transform.vs")
                               (fragment-shader-file "src/1.getting_started/5.1.transformations/5.1.transform.fs")
      ...
      (loop until (cl-glfw3:window-should-close-p)
          do (progn
              ...
              (let* ((time (%cl-glfw3:get-time))
                     (transform (3d-matrices:marr4
                                  (3d-matrices:nmtranslate
                                    (3d-matrices:nmrotate (3d-matrices:meye 4) 3d-vectors:+vz+ time)
                                    (3d-vectors:vec 0.5 -0.5 0.0)))))
                (use our-shader)
                ;; get matrix's uniform location and set matrix
                (gl:uniform-matrix-4fv
                    (gl:get-uniform-location (id our-shader) "transform")
                    transform t))
